...
describe('Test the "/geolocation/:location" path', () => {
  it('should response the GET method', async () => {
    ...
    expect(result.body[0].address)
      .toBe('Kharkiv, Kharkiv Oblast, Ukraine');
    expect(result.body[0].coordinates)
  ...
  it('should response the GET method', async () => {
    ...
    expect(result.body.message)
      .toBe('Entered address does not have any matches');
  });
});
...
describe('Test the "/geolocation/:lat/:lon" path', () => {
  it('should response the GET method', async () => {
    ...
    expect(result.body[0].address)
      .toBe("Unnamed Road, Kyivs'ka oblast, Ukraine");
    expect(result.body[0].coordinates)
      .toEqual({ lat: 49.999137, lon: 30.0019538 });
  });

  it('should response the GET method', async () => {
    const result = await simulate.get('/geolocation/444/444', 200);
    expect(result.body.message)
      .toBe('Response status code is 400');
  });
});
